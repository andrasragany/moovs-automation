...
public class Basic_test_Trainer_Fiskars{
    ...
    public static void main(String[] argv) throws Exception
    {
        ...
        Basic_test_Admin_Fiskars.createcommunication(logger, webDriver, wait, userGroupName1);
        //todo creating comm for trainer group. userGroupName1 will be the name of the comm for trainer group
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Fiskars.selector_trainer_add_LP_name, wait))
            webDriver.findElement(By.xpath(Object_repo_Fiskars.selector_trainer_add_LP_name)).sendKeys(userGroupName2);

        if (faszaklikk(webDriver, logger, Object_repo_Fiskars.selector_trainer_add_LP_description, wait))
            webDriver.findElement(By.xpath(Object_repo_Fiskars.selector_trainer_add_LP_description)).sendKeys(userGroupName2);
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Fiskars.selector_trainer_LP_search_group, wait))
            webDriver.findElement(By.xpath(Object_repo_Fiskars.selector_trainer_LP_search_group)).sendKeys(userGroupName2);
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Fiskars.selector_trainer_LP_search_for_path, wait))
            webDriver.findElement(By.xpath(Object_repo_Fiskars.selector_trainer_LP_search_for_path)).sendKeys(userGroupName2);
