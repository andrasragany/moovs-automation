...
public class Basic_test_Trainer_Philips{
    ...
    private static String create_comm(WebDriver webDriver, Logger logger, WebDriverWait wait, String string) throws ParseException, InterruptedException {
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Philips.selector_trainer_comm_search_group, wait))
            webDriver.findElement(By.xpath(Object_repo_Philips.selector_trainer_comm_search_group)).sendKeys(string);
    ...
    public static void main(String[] argv) throws Exception
    {
        ...
        String userGroupName1 = Basic_test_Admin_Philips.createstudentusergroup(logger, webDriver, wait, "Quince Trainer");
        //todo create trainer group for quince trainer for comm

        //todo create student group for quince student for training
        Basic_test_Admin_Philips.navigatetousergroups(logger, webDriver, wait, "trainer");
        String userGroupName2 = Basic_test_Admin_Philips.createstudentusergroup(logger, webDriver, wait, "Quince Student");
        //todo create student group for quince student for training
        ...
        Basic_test_Admin_Philips.createcommunication(logger, webDriver, wait, userGroupName1);
        //todo creating comm for trainer group. userGroupName1 will be the name of the comm for trainer group
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Philips.selector_trainer_add_LP_name, wait))
            webDriver.findElement(By.xpath(Object_repo_Philips.selector_trainer_add_LP_name)).sendKeys(userGroupName2);

        if (faszaklikk(webDriver, logger, Object_repo_Philips.selector_trainer_add_LP_description, wait))
            webDriver.findElement(By.xpath(Object_repo_Philips.selector_trainer_add_LP_description)).sendKeys(userGroupName2);
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Philips.selector_trainer_LP_search_group, wait))
            webDriver.findElement(By.xpath(Object_repo_Philips.selector_trainer_LP_search_group)).sendKeys(userGroupName2);
        ...
        if (faszaklikk(webDriver, logger, Object_repo_Philips.selector_trainer_LP_search_for_path, wait))
            webDriver.findElement(By.xpath(Object_repo_Philips.selector_trainer_LP_search_for_path)).sendKeys(userGroupName2);
